...
if __name__ == '__main__':
    TEST = False
    np.random.seed(0)
    model = torch.load("/home/saz/Desktop/qsrl_test/train_dir/attention_16_16_tanh/checkpoint_p0/best_000307732_315117568_reward_2.162.pth", map_location='cpu')
    ...
    while kk < iter:
        ...
        for n in range(z,z+1):
            ...
            for k in range(6):
                ...
                out_C_2[k] = actor['actor_encoder.neighbor_encoder.attention_mlp.4.weight'] @ out_C_1 + actor['actor_encoder.neighbor_encoder.attention_mlp.4.bias']
...
#exit()
            ...
            for q in range(6):
                ...
                for k in range(out_C_0_fix.shape[0]):
                    for u in range(6):
                        out_D_0_fix[k][q] = ((actor_fix['actor_encoder.neighbor_encoder.attention_mlp.0.weight'][k][u] *
                                              B_K_fix[q][
                                                  u]) * (2 ** -n)).astype(np.int32) + out_D_0_fix[k][q]
                    out_D_0_fix[k][q] = out_D_0_fix[k][q] + \
                                        actor_fix['actor_encoder.neighbor_encoder.attention_mlp.0.bias'][k]
                    out_D_0_fix[k][q] = tanh_fix_approx_johan_heinrich(out_D_0_fix[k][q],n)

                for k in range(out_D_1_fix.shape[0]):
                    for u in range(16):
                        out_D_1_fix[k][q] = (actor_fix['actor_encoder.neighbor_encoder.attention_mlp.2.weight'][k][u] *
                                             out_D_0_fix[k][q] * (2 ** -n)).astype(np.int32) + out_D_1_fix[k][q]
                    out_D_1_fix[k][q] = out_D_1_fix[k][q] + \
                                        actor_fix['actor_encoder.neighbor_encoder.attention_mlp.2.bias'][k]
